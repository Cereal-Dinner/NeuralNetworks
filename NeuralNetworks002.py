...
class NeuralTeacherTanh:
    ...
    def GetDeltaOfNeuron(inputs, weights, error):
        dWeights = [0.0 for i in range(len(weights))]
        tempInputs = inputs.copy()
        tempInputs.insert(0,1)
        for i in range(len(weights)):
            dWeights[i] += (tempInputs[i] * (1 - NeuralTeacherTanh.CalculateNeuronOutput(inputs,weights))) * error * 0.3
        return dWeights
    def CalculateNeuronOutput(inputs, weights):
        tempInputs = inputs.copy()
        tempInputs.insert(0,1)
        sum = 0
        for i in range(len(weights)):
